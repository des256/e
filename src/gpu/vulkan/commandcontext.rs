...
impl<'system> Window<'system> {
    ...
    pub fn acquire_next_context(&self,signal_semaphore: &Semaphore) -> CommandContext {
        let mut index = 0u32;
        unsafe { sys::vkAcquireNextImageKHR(self.system.vk_device,self.vk_swapchain,0xFFFFFFFFFFFFFFFF,signal_semaphore.vk_semaphore,null_mut(),&mut index) };
        CommandContext {
            window: &self,

// convert Expr::AnonTuple into Expr::Struct
...
pub(crate) fn make_anon_tuple_struct(module: &sr::Module,anon_tuple_structs: &mut HashMap<String,Vec<(String,sr::Type)>>,types: Vec<sr::Type>) -> String {
    let current_length = anon_tuple_structs.len();
    for (ident,fields) in &*anon_tuple_structs {
    ...
    let ident = format!("AnonTuple{}",module.anon_tuple_structs.len() + current_length);
    anon_tuple_structs.insert(ident.clone(),fields);
    ident
}
...
impl<'module> Resolver<'module> {
    ...
    fn expr(&mut self,expr: &sr::Expr) -> sr::Expr {
        match expr {
            ...
            sr::Expr::Ident(ident) => sr::Expr::Ident(ident.clone()),
            ...
            sr::Expr::AnonTuple(exprs) => {
                ...
                for expr in exprs {
                    types.push(infer_expr_type(self.module,&mut self.anon_tuple_structs,expr));
                }
                let ident = make_anon_tuple_struct(self.module,&mut self.anon_tuple_structs,types);
                let mut fields: Vec<(String,sr::Expr)> = Vec::new();
                for i in 0..exprs.len() {
                    fields.push((format!("_{}",i),self.expr(&exprs[i])))
                }
                sr::Expr::Struct(ident,fields)
            },
        ...
        match stat {
            sr::Stat::Let(ident,ty,expr) => {
                ...
                if let Some(ty) = &new_ty {
                    self.locals.insert(ident.clone(),ty.clone());
                }
                else {
                    let ty = infer_expr_type(self.module,&mut self.anon_tuple_structs,expr);
                    self.locals.insert(ident.clone(),ty);
                }
    ...
    fn type_(&mut self,ty: &sr::Type) -> sr::Type {
        match ty {
            ...
            sr::Type::Ident(ident) => sr::Type::Ident(ident.clone()),
...
pub fn resolve_anon_tuples(module: &sr::Module) -> sr::Module {
    ...
    // process anon tuples from gpu_macros
    ...
    // process anon tuples added here

...
fn draw_rgb<T: Pixel>(image: &mut Mat<T>,px: usize,py: usize,r: i32,g: i32,b: i32) {
...
fn draw_yuv<T: Pixel>(image: &mut Mat<T>,px: usize,py: usize,y: i32,u: i32,v: i32) {
	let r = ((y << 8) + 359 * v) >> 8;
...
fn draw_macroblock_y<T: Pixel>(image: &mut Mat<T>,x0: usize,y0: usize,width: usize,height: usize,coeffs: &[i32]) {
	for i in 0..height {
		for k in 0..width {
...
fn draw_macroblock_yuv420<T: Pixel>(image: &mut Mat<T>,x0: usize,y0: usize,width: usize,height: usize,coeffs: &[i32]) {
	for i in 0..height {
		for k in 0..width {
			let by = (i >> 3) * 2 + (k >> 3);
			let si = i & 7;
...
fn draw_macroblock_yuv422<T: Pixel>(image: &mut Mat<T>,x0: usize,y0: usize,width: usize,height: usize,coeffs: &[i32]) {
	for i in 0..height {
		for k in 0..width {
			let by = k >> 3;
			let sk = k & 7;
...
fn draw_macroblock_yuv440<T: Pixel>(image: &mut Mat<T>,x0: usize,y0: usize,width: usize,height: usize,coeffs: &[i32]) {
	for i in 0..height {
		for k in 0..width {
			let by = i >> 3;
			let si = k & 7;
...
fn draw_macroblock_yuv444<T: Pixel>(image: &mut Mat<T>,x0: usize,y0: usize,width: usize,height: usize,coeffs: &[i32]) {
	for i in 0..height {
		for k in 0..width {
...
fn draw_macroblock_rgb444<T: Pixel>(image: &mut Mat<T>,x0: usize,y0: usize,width: usize,height: usize,coeffs: &[i32]) {
	for i in 0..height {
		for k in 0..width {
...
pub fn decode<T: Pixel + Default>(src: &[u8]) -> Option<Mat<T>> {
	...
	while sp < src.len() {
		...
		match marker {
			...
			0xFFD9 => {  // image end
				//println!("end");
				let mut image = Mat::new(Vec2::<usize> { x: width,y: height, });
				match itype {
...
pub fn encode<T: Pixel>(_image: &Mat<T>) -> Option<Vec<u8>> {
	None
}

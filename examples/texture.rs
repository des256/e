...
#[vertex_shader]
mod quad_vs {
    fn main(vertex: MyVertex) -> (Vec4<f32>,Vec2<f32>) {
        (Vec4::<f32> { x: vertex.pos.x,y: vertex.pos.y,z: 0.0,w: 1.0, },vertex.tc)
    }
}

#[fragment_shader]
mod quad_fs {
    const image: Sampler2d = 0;
    fn main(tc: Vec2<f32>) -> Vec4<f32> {
        let color = texture(image,tc);
...
impl<T: Gpu> Environment<T> {
    ...
    fn new(system: &Rc<System>,gpu: &Rc<T>,ext: &'static str,title: &'static str,r: Rect<i32>) -> Result<Environment<T>,String> {
        ...
        vertices.push(MyVertex { pos: Vec2::<f32> { x: -0.5,y: -0.5, }, tc: Vec2::<f32> { x: 0.0,y: 0.0, }, });
        vertices.push(MyVertex { pos: Vec2::<f32> { x: 0.5,y: -0.5, }, tc: Vec2::<f32> { x: 1.0,y: 0.0, }, });
        vertices.push(MyVertex { pos: Vec2::<f32> { x: 0.5,y: 0.5, }, tc: Vec2::<f32> { x: 1.0,y: 1.0, }, });
        vertices.push(MyVertex { pos: Vec2::<f32> { x: -0.5,y: 0.5, }, tc: Vec2::<f32> { x: 0.0,y: 1.0, }, });
